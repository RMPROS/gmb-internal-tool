...
async function fetchPlacesData(bizName, placeId) {
  ...
  // Also get the raw placeId without prefix for old API fallback
  const rawPlaceId = resolvedPlaceId.replace('places/', '');
  ...
  try {
    ...
    // If New API returned no photos, fall back to Old API just for photo count
    if (!data.photos || data.photos.length === 0) {
      try {
        const oldDetailUrl = 'https://maps.googleapis.com/maps/api/place/details/json'
          + '?place_id=' + rawPlaceId
          + '&fields=photos'
          + '&key=' + apiKey;
        const oldRes  = await fetch(oldDetailUrl);
        const oldData = await oldRes.json();
        if (oldData.result?.photos?.length) {
          console.log('[PLACES] Photo fallback: old API returned', oldData.result.photos.length, 'photos');
          data.photos = oldData.result.photos; // inject old API photos into new API result
        }
      } catch(fe) {
        console.error('[PLACES] photo fallback error:', fe.message);
      }
    }
