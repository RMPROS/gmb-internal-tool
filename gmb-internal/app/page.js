...
function RunAudit() {
  ...
  const onSearch = (v) => {
    setQuery(v); setSelected(null); setResult(null); setCleanName('');
  ...
  const runIt = async () => {
    ...
    try {
      ...
      if (d.cleanBizName) { setCleanName(d.cleanBizName); setSelected(prev => ({ ...prev, name: d.cleanBizName })); }
  ...
  return (
    <div>
      ...
      {result && <AuditResult result={result} bizName={cleanName || selected?.name || query} />}
    </div>
  );
}
