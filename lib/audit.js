...
export function formatPlacesDataForDebug(p) { return formatPlacesData(p); }
...
function formatPlacesData(p) {
  ...
  const description    = ownDescription
    ? ownDescription
    : aiDescription
      ? '[Owner description not returned by API. Google AI summary shown below. Tell them to use the DESCRIPTION_TOOL_LINK to create a strong owner description]: ' + aiDescription
      : 'No description detected by the Google Places API. Tell them to use the DESCRIPTION_TOOL_LINK to elevate their description.';
...
function injectDescLink(result) {
  const MARKER = 'DESCRIPTION_TOOL_LINK';
  const LINK   = '<a href="https://desc.rentalmarketingpros.com/" style="color:#FF8C00;font-weight:700;text-decoration:underline;">Description Optimization Tool</a>';
  if (!result?.sections) return result;
  result.sections.forEach(section => {
    if (!section.findings) return;
    section.findings.forEach(finding => {
      if (finding.desc && finding.desc.includes(MARKER)) {
        finding.desc = finding.desc.replace(new RegExp(MARKER, 'g'), LINK);
      }
    });
  });
  return result;
...
export async function runAudit(bizName, placeId) {
  ...
  // Use the clean business name from the API, apply title case
  const rawBizName    = placesData?.displayName?.text || bizName;
  const cleanBizName  = rawBizName.split(' ').map(w =>
    w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()
  ).join(' ');
  const realDataBlock = formatPlacesData(placesData);

  const dataSection = realDataBlock
    ? 'CRITICAL INSTRUCTION: The following data was pulled LIVE from the Google Places API right now. You MUST base every single finding strictly on these exact numbers. Do NOT contradict this data. Do NOT say there are no reviews if Total Reviews shows a number. Do NOT say there are no photos if Photo Count shows a number. Do NOT say there is no description if Description shows text. Use the EXACT numbers provided.\n\n' + realDataBlock + '\n\n'
  ...
  const prompt = 'You are a Google My Business (GMB) local SEO expert auditing the profile for "' + cleanBizName + '".\n\nIMPORTANT: You will be given REAL live data from the Google Places API. Your findings MUST accurately reflect this data. Never contradict the provided data — if it says 5 reviews, your findings must reference 5 reviews. If it says 5 photos, your findings must reference 5 photos.\n\n'
    + dataSection
