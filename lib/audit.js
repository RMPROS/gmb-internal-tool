...
function formatPlacesData(p) {
  ...
  const description    = ownDescription
    ...
    : aiDescription
      ...
      : 'Not returned by API - business may have a description set, verify manually in Google Business Profile';
...
export async function runAudit(bizName, placeId) {
  ...
  // Use the clean business name from the API if available
  const cleanBizName  = placesData?.displayName?.text || bizName;
  ...
  const prompt = 'You are a Google My Business (GMB) local SEO expert auditing the profile for "' + cleanBizName + '".\n\nIMPORTANT: You will be given REAL live data from the Google Places API. Your findings MUST accurately reflect this data. Never contradict the provided data — if it says 5 reviews, your findings must reference 5 reviews. If it says 5 photos, your findings must reference 5 photos.\n\n'
    ...
    + '- If Description contains "not returned by API" or "verify manually", do NOT score it as critical — note it could not be verified and tell them to check it manually in their Google Business Profile\n'
    + '- If Description contains "Google AI summary", note their owner description was not detected but include the AI summary as context, and tell them to verify/update their own description\n'
